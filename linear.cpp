...
int parent_mapping(int q)
{
	std::ifstream infile("parent.txt");
	std::string line;
	int cnt=0;
...
double l2r_lr_fun::fun(double *w)
{
	...
	for(i=0;i<w_size;i++)
	f += (w[i]-w_pi[(parent_mapping(current_n)*N)+i])*(w[i]-w_pi[(parent_mapping(current_n)*N)+i]);
	f /= 2.0;
	for(i=0;i<l;i++)
	{
	double yz = y[i]*z[i];
...
void l2r_lr_fun::grad(double *w, double *g)
{
	...
	for(i=0;i<w_size;i++)
	g[i] = w[i] - w_pi[parent_mapping(current_n)*N + i]+ g[i];
}
	...
	double Cp, double Cn, int solver_type)
{
	...
	double *wtemp=(double *)malloc(w_size*sizeof(double));
	for(int k=0;k<w_size;k++)
		wtemp[k]=w_pi[parent_mapping(current_n)*N+k];
	G += yi*sparse_operator::dot(wtemp, xi);
	PG = 0;
	if (alpha[i] == 0)
	{
	if (G > PGmax_old)
	...
	w[j]+=w_pi[parent_mapping(current_n)*N + j];
	}
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	break;
	default:
	fprintf(stderr, "ERROR: unknown solver_type 2261\n");
	break;
	}
}
...
struct parameter * copy_parameters(parameter *param)
{
...
// Interface functions
//
int index_mapping(int q)
{
	std::ifstream infile("real.txt");
	std::string line;
	while (std::getline(infile, line))
	{
	std::istringstream iss(line);
...
{
	print_string_stdout("Reached the start of train");
	int i,j;
	int l = prob->l;
	int n = prob->n;
	int w_size = prob->n;
	...
	print_string_stdout("Finished printing classes");
	...
	print_string_stdout("Reached before conditions");
	...
	else if((param->solver_type == L2R_HR_LR)||(param->solver_type == L2R_L1LOSS_HR_SVC_DUAL))
	{
		print_string_stdout("Found solver");
		std::string line;
		int levels[300][300];
		N=0;
		for(int i=0;i<100;i++)
			for(int j=0;j<100;j++)
				levels[i][j]=0;
		int level_index=0,e=0,n_levels;
	...
	print_string_stdout("Read levels file");
	n_levels=level_index+1;


	for(int ii=0;ii<N*w_size;ii++)
		w_pi[ii]=0;
//	struct parameter p1;
	//struct problem pr1;
	print_string_stdout("Initiliazed w_pi");
	hr_flag=1;
	for (level_index = 0; level_index < n_levels; level_index++)
	{
		...
		for (i=0;i<prob->l;i++)
		{
			int ps_l=index_mapping(prob->y[i]);
			if(ps_l<nr_class)
			{
				int old=ps_l,flag=0;
				int l_index=1;
				while(l_index<=level_index)
				{
					int p=parent_mapping(old);
					for(int k=0;levels[l_index][k]!=0;k++)
					{
					if(levels[level_index][k]==p)
					y[i]=levels[level_index][k];
					flag=1;
					break;
					}
		...
		print_string_stdout("Yexchange for all samples");
...
static const char *solver_type_table[]=
{
	...
	"", "", "",
	"L2R_L2LOSS_SVR", "L2R_L2LOSS_SVR_DUAL", "L2R_L1LOSS_SVR_DUAL", "L2R_HR_LR","L2R_L1LOSS_HR_SVC_DUAL",NULL
};
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	&& param->solver_type != L2R_HR_LR
	&& param->solver_type != L2R_L1LOSS_HR_SVC_DUAL)
	return "unknown solver type 3220";
...
int **read_csv_mapping(char* filename)
{
